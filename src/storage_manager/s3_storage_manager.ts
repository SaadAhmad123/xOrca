...
export default class S3StorageManager extends StorageManager {
    ...
    async read(path: string, __default: string = ""): Promise<string> {
        try {
            const response = await this.s3.getObject({ Bucket: this.bucketName, Key: path }).promise();
    ...
    async write(data: string, path: string): Promise<void> {
        await this.s3.putObject({ Body: data, Bucket: this.bucketName, Key: path }).promise();
        console.log(`File ${path} has been written successfully in bucket ${this.bucketName}.`);
    }
    ...
    async exists(path: string): Promise<boolean> {
        try {
            await this.s3.headObject({ Bucket: this.bucketName, Key: path }).promise();
            return true;
        } catch (error) {
